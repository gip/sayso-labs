...
export const SAYSO_PONG_AUTHORITY = "sayso.demo.pong";
...
export const CONTENT_TYPES = {
  ...
  pingRequest: makeContentType(SAYSO_PONG_AUTHORITY, "ping-request"),
  pongResponse: makeContentType(SAYSO_PONG_AUTHORITY, "pong-response"),
...
export const contentTypeFromName = (name: string): ContentTypeId | null => {
  for (const value of Object.values(CONTENT_TYPES)) {
    if (contentTypeName(value) === name) return value;
  }
  return null;
};
